...
//渠道出价 单位：分 [该渠道为客户端bidding模式下，返回渠道出价，非客户端bidding模式下，返回美数平台设置的出价]
extern NSString *const kMSAdMediaAdnEcpmKey;
//美数平台价格 单位：分 [获取该渠道在美数平台设置的出价]
 ...
 *  MSBaseAd.h中有定义）为以下内容
 *  kMSAdMediaWinPrice ：竞胜价格 (单位: 分)。值类型为NSString
 *  kMSAdMediaLossReason ：广告竞败原因。竞败原因参考枚举
 *  MSMediaLossReason中的定义，值类型为NSString
 *
 *  kMSAdMediaWinADN ：竞胜方渠道ID。渠道ID参考枚举
 *  MSMediaWinADN中的定义，值类型为NSString
 *  
